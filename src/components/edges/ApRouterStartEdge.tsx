...
const BranchLabel: React.FC<BranchLabelProps> = ({ label }) => {
  return (
    <div className='h-full flex items-center justify-center'>
      ...
      >
        <div
          className='flex items-center justify-center gap-0.5 select-none transition-all rounded-full text-sm border border-solid bg-primary-100/30 dark:bg-primary-100/15 border-primary/50 px-2 text-primary/80 dark:text-primary/90 hover:text-primary hover:border-primary bg-white'
          style={{
            height: flowConstants.LABEL_HEIGHT + 'px',
            maxWidth: flowConstants.AP_NODE_SIZE.step.width - 10 + 'px',
          }}
        >
          <div className='truncate'>{label === 'Otherwise' ? 'Otherwise' : label}</div>
        </div>
      </div>
    </div>
  )
...
export const ApRouterStartEdge = ({ sourceX, sourceY, targetX, targetY, data, source, target, id }: EdgeProps) => {
    ...
    if (distanceBetweenSourceAndTarget >= flowConstants.ARC_LENGTH) {
      ...
      if (routerData.drawHorizontalLine) {
        ...
        const horizontalLength =
          (Math.abs(targetX - sourceX) + 3 - 2 * flowConstants.ARC_LENGTH) * (sourceX > targetX ? 1 : -1)
      ...
      if (routerData.drawStartingVerticalLine) {
        ...
        const finalVerticalLength =
          flowConstants.VERTICAL_SPACE_BETWEEN_STEPS / 2 - 2 * flowConstants.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE
  ...
  return (
    <>
      ...
      <foreignObject
        ...
        height={flowConstants.LABEL_HEIGHT + flowConstants.LABEL_VERTICAL_PADDING + 'px'}
        x={targetX - (flowConstants.AP_NODE_SIZE.step.width - 10) / 2}
        y={targetY - verticalLineLength / 2 - flowConstants.AP_NODE_SIZE.ADD_BUTTON.height - 40}
