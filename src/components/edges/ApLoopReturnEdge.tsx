...
export const ApLoopReturnEdge = ({
  ...
  const horizontalLineLength =
    Math.abs(sourceX - targetX) - 2 * flowConstants.ARC_LENGTH

  const verticalLineLength =
    loopData.verticalSpaceBetweenReturnNodeStartAndEnd -
  ...
  const path = `
  M ${sourceX - 0.5} ${
    sourceY - flowConstants.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE * 2 - 1
  }
  v 1
  ...
  M ${sourceX - flowConstants.ARC_LENGTH - horizontalLineLength / 2} ${
   ...
   v${endLineLength} ${
    loopData.drawArrowHeadAfterEnd ? flowConstants.ARROW_DOWN : ''
  } 
   `
