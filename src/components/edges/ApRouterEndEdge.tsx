...
export const ApRouterEndEdge = ({ sourceX, sourceY, targetX, targetY, data }: EdgeProps) => {
  ...
  const horizontalLineLength =
    (Math.abs(targetX - sourceX) - 2 * flowConstants.ARC_LENGTH) * (targetX > sourceX ? 1 : -1)

  const distanceBetweenTargetAndSource = Math.abs(targetX - sourceX)

  const generatePath = () => {
    // Start point
    let path = `M ${sourceX - 0.5} ${sourceY - flowConstants.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE * 2}`
    ...
    if (distanceBetweenTargetAndSource >= flowConstants.ARC_LENGTH) {
      path += targetX > sourceX ? flowConstants.ARC_RIGHT_DOWN : flowConstants.ARC_LEFT_DOWN
    } else {
      path += `v ${flowConstants.ARC_LENGTH + flowConstants.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE + 2}`
    }

    // Optional horizontal line
    if (routerData.drawHorizontalLine) {
      path += `h ${horizontalLineLength} ${targetX > sourceX ? flowConstants.ARC_RIGHT : flowConstants.ARC_LEFT}`
    }
  ...
  return (
    <>
      ...
      {routerData.drawEndingVerticalLine && routerData.isNextStepEmpty && (
        <AddButton
          x={targetX}
          y={targetY - verticalLineLength / 2 - flowConstants.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE}
