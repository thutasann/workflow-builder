...
export const LoopSettings: React.FC<LoopSettingsProps> = ({ step }) => {
  ...
  return (
    <div className='space-y-4'>
      <div>
        <label className='block text-sm font-medium text-gray-700 mb-2'>Items to Loop Over</label>
        <input
          type='text'
      ...
      <div className='bg-blue-50 border border-blue-200 rounded-md p-3'>
        ...
        <ul className='text-xs text-blue-700 space-y-1'>
          ...
          <li>
            • Access the current item with <code className='bg-blue-100 px-1 rounded'>{'{{loopItem}}'}</code>
          </li>
          <li>
            • Access the current index with <code className='bg-blue-100 px-1 rounded'>{'{{loopIndex}}'}</code>
          </li>
        </ul>
      </div>
      ...
      <div className='border-t pt-4'>
        <p className='text-xs text-gray-500'>
          The loop will execute the actions inside for each item in the array. If the array is empty, the loop will be
          skipped.
        </p>
      </div>
    </div>
